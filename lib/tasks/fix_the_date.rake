...
def process_the_data model, key
  ...
  if Setting.find_by_name("host_name").nil?
    default_url_options[:host] = YAML::load(File.open("#{RAILS_ROOT}/config/settings.yml"))["host_name"]["default"]
  else
    default_url_options[:host] = Setting.find_by_name("host_name").value
  end
  ...
  model.all.each do |one_record|
    ...
    if matches
      if model==Journal then
        index = Issue.find_by_id(one_record.journalized_id).journals.find(:all, :order => "#{Journal.table_name}.created_on ASC").find_index(one_record)+1
        url= (url_for :controller => 'issues',:action => 'show',:id => one_record.journalized_id, :anchor => "note-#{index}")
      else
        url= url_for :controller => 'issues',:action => 'show',:id => one_record
      end
